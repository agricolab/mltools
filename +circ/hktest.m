function [pval table] = hktest(alpha, idp, idq, inter, fn)

%
% [pval, stats] = circ.hktest(alpha, idp, idq, inter, fn)
...
for pp = 1:p
    ...
    for qq = 1:q
        ...
        cr(pp,qq) = cn(pp,qq) * circ.r(alpha(idx)); % R of cell
    end
    % R and mean angle for factor 1
    pr(pp) = sum(p_id) * circ.r(alpha(p_id));
    pm(pp) = circ.mean(alpha(p_id));
    pn(pp) = sum(p_id);
end
...
for qq = 1:q
    q_id = idq == qu(qq);
    qr(qq) = sum(q_id) * circ.r(alpha(q_id));
    qm(qq) = circ.mean(alpha(q_id));
    qn(qq) = sum(q_id);
end
