function [b b0] = skewness(alpha, w, dim)

% [b b0] = circ.skewness(alpha,w,dim)
...
%     left empty: circ.skewness(alpha, [], dim)
%
%   Output:
...
[~, rho2 mu2] = circ.moment(alpha,w,2,false,dim);

% compute skewness 
theta2 = repmat(theta, size(alpha)./size(theta));
b = sum(w.*(sin(2*(circ.dist(alpha,theta2)))),dim)./sum(w,dim);
b0 = rho2.*sin(circ.dist(mu2,2*theta))./(1-R).^(3/2);    % (formula 2.29)
