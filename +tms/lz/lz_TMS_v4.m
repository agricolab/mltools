%{
    ...
    Email                : lukas.ziegler@uni-tuebingen.de/ziegler.swv@gmail.com
    ...
    Description          : This is a full-fledged function to control various TMS stimulation techniques
                           Possible Determinations:
                            ...
                            - iMEP with different coil directions
...
classdef lz_TMS_v4 < handle

    properties
        marker_pin      = [1, 5, 9]; % Pin to only add marker to buffer
        stim_pin        = 3;      % Pin to stimulate and add marker to buffer
        lpt_ad          = 'LPT2'; % Parallel port to use for stimulation
                    ...
                    switch obj.objMag.spObject.BytesAvailable
                        case 8
                            ...
                            if strcmpi(mag_fbk, 'stimulate') %Manual MagVenture Stimulation
                                ...
                                obj.objMag.changeAmp(0,0);
                                if obj.MEP.maxAmp(run,rep)+obj.MEP.minAmp(run,rep) > 50
                                    amp = input('MEP detected.\nChoose next intensity: ');
                                else
                                    amp = input('NO MEP detected.\nChoose next intensity: ');
                                end
                                run = run + 1;
                                if run > 20
                                    finish = 0;
...
%             keyboard
        ...
        function get_MVC(obj)
            ...
            % steps
            obj.mvc = []; %Clear previous data
            obj.check_buffer() %Check if buffer is still recording
            obj.objMag.turnOnOff('off');
            obj.objMag.flushData();
            obj.dataEEGEMG = [];
            time = 'pre';
            if exist([obj.directories.mvc 'MVC_' time '.mat'],'file') == 2
                time = input('Time: ','s');
                if exist([obj.directories.mvc 'MVC_' time '.mat'],'file') == 2
                    time = input('File exists already. Redo Time: ','s');
                end
            ...
            while toc(run_tic) < 2      %Measure pre MVC for 2s
                if toc(step_tic) >= 0.2 %take last 200ms
                    ...
                    obj.PTB.m_contr(pre_contr(step_rise), tmp, tmp,obj.tms_settings.mvc_thresh_range,0.8)
                    clear tmp
            ...
            while toc(run_tic) < 5  %Measure MVC for 5s in 200ms steps
                if toc(step_tic) >= 0.2 %take last 200ms
                    ...
                    % (current contraction force, MVC, threshold,
                    % threshold_range,zoom_factor)
                    obj.PTB.m_contr(obj.mvc.step(step), obj.mvc.mean,obj.mvc.mean, obj.tms_settings.mvc_thresh_range,0.8)
        ...
        function gep_opt_ipi_ipsi(obj, feedback_muscle)
            ...
            time = 'pre';
            if exist([obj.directories.imep 'opt_ipi_' time '.mat'],'file') == 2
                time = input('Time: ','s');
                if exist([obj.directories.imep 'opt_ipi_' time '.mat'],'file') == 2
                    time = input('File exists already. Redo Time: ','s');
                end
                                ...
                                while finish
                                    ...
                                    for rep = 1:obj.tms_settings.imep_iterations  %apply x stimuli for each IPI
                                        ...
                                        if(~isempty(obj.dataEEGEMG))
                                            ...
                                            if isempty(stimTime); stimTime = NaN; end
                                            if(stimTime > 40 && ~isnan(stimTime))
                                                obj.tms_settings.targetmuscle       = feedback_muscle;
                                                data = obj.filterData(obj,obj.tms_settings.addNotchFilt);
                                                obj.tms_settings.targetmuscle       = 'BB_L';
                                    ...
                                    cla(h2);
                                    plot(h2,sum(obj.MEP.iMEP(1:ipi_n,:),2),'ok','MarkerSize',10,'MarkerFaceColor','k')
                                    xticks(h2,1:ipi_n)
                                    ...
                                    cla(h3);
                                    plot(h3,mean(obj.MEP.peakToPeak(1:ipi_n,:),2),'ok','MarkerSize',10,'MarkerFaceColor','k')
                                    xticks(h3,1:ipi_n)
                                    xticklabels(h3,obj.tms_settings.ipi_imep(1:ipi_n))
                                    ylabel(h3, '\Delta iMEP')
                                    drawnow
            ...
            % Plot the mean MEPs
            figure
            x = 1:numel(obj.tms_settings.ipi_imep);
            tmp = obj.MEP.peakToPeak/nanmean(obj.MEP.peakToPeak(obj.tms_settings.ipi_imep==0,:),2)*100;
            plot(x,nanmean(tmp,2),'s','Color',[0 0 0],'LineWidth',2,'LineStyle','none','MarkerFaceColor',[0 0 0])
            ax = gca;
            ax.XTick=x;
            set(gca,'XTickLabel',obj.tms_settings.ipi_imep(x),'Fontsize',12,'Fontweight','bold')
            title('Normalized MEP', 'FontSize', 16)
            xlabel('ISI [ms]')
            ylabel('\Delta iMEP [% SP]')
...
%                                 h2=subplot(2,1,2);
        ...
        function iMEP_test(obj,feedback_muscle)
            % Function to determine if iMEP is detectable after application
            % of 10 stimuli at given amp with set % of MVC
            obj.check_buffer() %Check if buffer is still recording
            obj.objMag.turnOnOff('on'); %Turn MagVen on
            obj.objMag.changeAmp(0,0); %change magventure to 0%
            obj.dataEEGEMG = []; % Clear data matrix
            amp = obj.tms_settings.imep_amp; %Stimulation intensity
            if exist([obj.directories.imep 'imep_test_' feedback_muscle '.mat'],'file') == 2
                feedback_muscle = input('Set feedback muscle: ','s');
                if exist([obj.directories.imep 'imep_test_' feedback_muscle '.mat'],'file') == 2
                    feedback_muscle = input('File exists already. Set feedback muscle: ','s');
                end
            end
            obj.objMag.flushData();
            finish  = 1;
            ...
            disp('Saving...')
            if ~isdir(obj.directories.imep); mkdir(obj.directories.imep); end
            save([obj.directories.imep 'imep_test_' feedback_muscle], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.changeAmp(0,0)
        end
        ...
        %% Functions for EEG measurements
        function restEEG(obj, duration)
            clc
            data_tmp = [];
            time = 'pre';
            if exist([obj.directories.EEG 'restingEEG_' time '.mat'],'file') == 2
                time = input('Time: ','s');
                if exist([obj.directories.EEG 'restingEEG_' time '.mat'],'file') == 2
                    time = input('File exists already. Redo Time: ','s');
                end
            ...
            obj.check_buffer();
            % First show text 'Entspannen' and wait for 3 s
            obj.add_marker(obj,1)       %adds marker to buffer
            obj.PTB.displayMessage('ENTSPANNEN')
            pause(3)
            % Read out data and store it as buffer can't hold for longer
            % than 3 s
            tmp = double((obj.objBuf.read_data())/10); % read out data
            data_tmp = cat(1,data_tmp,tmp);

            % Show Cross and wait for duration s
            obj.add_marker(obj,2)       %adds marker to buffer
            obj.PTB.drawCross();
            step_tic = tic;
            duration_tic = tic;
            while toc(duration_tic) < duration %Stimulate if MVC +- range is reached
                if toc(step_tic) >= 1 %Read out data and store it every second
                    tmp = double((obj.objBuf.read_data())/10); % read out data
            ...
            % End EEG aquisition
            obj.add_marker(obj,3)       %adds marker to buffer
            obj.PTB.displayMessage('ENDE')
            pause(1)
            tmp = double((obj.objBuf.read_data())/10); % read out data
            data_tmp = cat(1,data_tmp,tmp); %append to data matrix

            % Save data
            obj.dataEEGEMG = data_tmp; % read out data
            if ~isdir(obj.directories.EEG); mkdir(obj.directories.EEG); end
            save([obj.directories.EEG 'restingEEG_' time], 'obj', '-v7.3')

        end

        function CMC(obj, duration)
            data_tmp = [];
            time = 'pre';
            if exist([obj.directories.EEG 'restingEEG_' time '.mat'],'file') == 2
                time = input('Time: ','s');
            ...
            % First show text 'Entspannen' and wait for 3 s
            obj.add_marker(obj,1)       %adds marker to buffer
            obj.PTB.displayMessage('Linke Hand')
            pause(3)
            tmp = double((obj.objBuf.read_data())/10); % read out data
            data_tmp = cat(1,data_tmp,tmp);

            % Show required contraction force and wait for duration s
            obj.PTB.displayMessage('Contract')
            play(obj.audio.contract);
            ...
            while toc(duration_tic) < duration %Stimulate if MVC +- range is reached
                if toc(step_tic) >= 0.2 %take last 200ms
                    ...
                    % (current contraction force, MVC, threshold,
                    % threshold_range,zoom_factor)
                    obj.PTB.m_contr(contr_force, obj.mvc.mean, obj.tms_settings.mvc_thresh, obj.tms_settings.mvc_thresh_range,1) %visual fdbk of contraction
                    clear tmp
                    step_tic = tic; %reset timer
                end
            end
            tmp = double((obj.objBuf.read_data())/10); %get data
            data_tmp = cat(1,data_tmp, tmp); %merge data
            play(obj.audio.relax);
            ...
            obj.dataEEGEMG = data_tmp; % read out data
            if ~isdir(obj.directories.EEG); mkdir(obj.directories.EEG); end
            save([obj.directories.EEG 'CMC_' time], 'obj', '-v7.3')
                            ...
                            if strcmpi(mag_fbk, 'stimulate') %Manual MagVenture Stimulation
                                ...
                                while finish
                                    ...
                                    for rep = 1:10
                                        if rep ~= 1
                                            %ISI of 5 +- 1.25 s
                                            while toc(isiTic) < (6.25-3.75)*rand(1)+3.75, end
                                        end
                                        display(['Rep: ' num2str(rep)])
                                        %stimulate with specified intensity if target muscle at rest and get buffer data
...
%                                 break;
        ...
        function iMEP_mvc_range(obj)
            ...
            amp = obj.tms_settings.imep_amp; %Stimulation intensity
            time = 'pre';
            if exist([obj.directories.imep 'mvc_thresh_range_' time '.mat'],'file') == 2
                time = input('Time: ','s');
                if exist([obj.directories.imep 'mvc_thresh_range_' time '.mat'],'file') == 2
                    time = input('File exists already. Redo Time: ','s');
                end
            ...
            obj.objMag.flushData();
            finish  = 1;
            round = 1; %number of current stimulus
            set_back_int = 0; %set back to previous stim if data is not recorded
            while finish
                if(obj.objMag.spObject.BytesAvailable~=0)
                    switch obj.objMag.spObject.BytesAvailable
                        case 8
                            ...
                            if strcmpi(mag_fbk,'stimulate')
                                ...
                                while finish
                                    obj.objMag.changeAmp(amp,0);
                                    pause(1)
                                    obj.tms_settings.mvc_range = obj.tms_settings.mvc_range(randperm(numel(obj.tms_settings.mvc_range))); %random order of MVCs
                                    for rep = 1:numel(obj.tms_settings.mvc_range)  %apply x stimuli for each MVC threshold
                                        if rep ~= 1
                                            %ISI of 5 +- 1.25 s
                                            while toc(isiTic) < (6.25-3.75)*rand(1)+3.75, end %500ms for MagVen to change int
                                        end
                                        % Subject has to contract muscle at certain threshold to trigger next step
                                        fprintf('Rep: %d \n % MVC to be reached: %3.1f \n', rep, obj.tms_settings.mvc_range(rep))
                                        % Set new threshold for contraction
                                        obj.tms_settings.mvc_thresh = obj.tms_settings.mvc_range(rep);
                                        obj.tms_settings.mvc_order(round,rep) = obj.tms_settings.mvc_range(rep); %store applied mvc
                                        obj.PTB.displayMessage('Left Hand');
                                        play(obj.audio.prep);
                                        pause(1)
                                        obj.PTB.displayMessage('Contract');
                                        ...
                                        if(~isempty(obj.dataEEGEMG))
                                            trig         = obj.dataEEGEMG(end-obj.ampSettings.SampRate+1:end,obj.ampSettings.ChanNumb+1)'; %last 2 seconds of marker channel
                                            stimTime     = find(trig, 1, 'last'); %Last entry of marker in marker channel
                                            stimTime     = stimTime + floor(0.001*obj.tms_settings.ipi_imep(round)*obj.ampSettings.SampRate); %so that the MEP triggered by the S2 is analyzed
                                            if isempty(stimTime); stimTime = NaN; end
                                            if(stimTime > 40 && ~isnan(stimTime))
                                                data = obj.filterData(obj,obj.tms_settings.addNotchFilt);
                                                obj.getiMEPinf(obj,data,stimTime,round,rep);
                                                obj.plotiMEP(obj, data, stimTime, rep, h1)
                                                obj.objLoc.sendMEP(obj.MEP.peakToPeak(round,rep),obj.MEP.peakToPeak(round,rep), obj.MEP.duration(round,rep));
                                            else
                                                obj.MEP.peakToPeak(round,rep)  = 0;
                                                obj.MEP.duration(round,rep)   = 0;
                                                obj.objLoc.sendMEP(0,0,0);
                                            end
                                        end
                                    end
                                    obj.tms_settings.mvc_range = obj.tms_settings.mvc_range(randperm(numel(obj.tms_settings.mvc_range))); %random order of MVCs
                                    % Sort according to MVC
                                    clear tmp
                                    for i = 1:numel(obj.tms_settings.mvc_range)
                                        a = reshape(obj.MEP.iMEP,1,[]);
                                        b = reshape(obj.tms_settings.mvc_order,1,[]);
                                        mvc_sorted = sort(obj.tms_settings.mvc_range);
                                        a(b ~= mvc_sorted(i))=0;
                                        tmp(i)=sum(a);
                                    end
                                    %Plot
                                    cla(h2);
                                    plot(tmp,'ok','MarkerSize',10,'MarkerFaceColor','k')
                                    xticks(h2,1:numel(obj.tms_settings.mvc_range))
                                    xticklabels(h2,mvc_sorted)
                                    xlabel(h2, 'Percentage of MVC')
                                    ylabel(h2, 'Number of iMEPs')
                                    title({'Round'; [num2str(round) ' out of ' num2str(obj.tms_settings.imep_iterations)]}, 'FontSize', 12)
                                    drawnow
                                    if round < obj.tms_settings.imep_iterations
                                        if set_back_int
                                            round = round - 1;
                                            set_back_int = 0;
                                        end
                                        round = round + 1;
                                        obj.pauseFunc(obj, 0, obj.tms_settings.ipi_imep(round));
                                    else
                                        finish = 0;
                                    end
                                end
            ...
            disp('Saving...')
            if ~isdir(obj.directories.imep); mkdir(obj.directories.imep); end
            save([obj.directories.imep 'mvc_thresh_range_' time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        ...
        function check_mvc(obj)
           tmp = load('E:\projects\lukasz\BT_iDIS\data\NiRe_08-Nov-2017\MVC\MVC_pre2.mat');
        end
    ...
    methods(Static, Access = private)
        ...
        function data=filterData(obj,addNotch)
            ...
            % Take last second of target muscle
        ...
        function getiMEPinf(obj, data, stimTime, int, rep)
            ...
            base_start = -100; %ms
            base_end   = 0; %ms
            base = abs(data(stimTime+(0.001*base_start*obj.ampSettings.SampRate):stimTime+(0.001*base_end*obj.ampSettings.SampRate)));
            base_mean = mean(base);
            base_sd   = std(base);
            base_max  = max(base);
            base_min  = min(base);
            base_ptp  = base_max + abs(base_min);
            % See if datapoint exceeds mean + 2SD
            x_start = obj.tms_settings.mep_start;    %ms; start of MEP search
            x_end   = obj.tms_settings.mep_end;  %ms; end of MEP search
            post = abs(data(stimTime+(0.001*x_start*obj.ampSettings.SampRate):stimTime+(0.001*x_end*obj.ampSettings.SampRate)));
            % Calculate how many seconds the postimulus EMG exceeded the
            % prestimulus mean by > 1 SD
            tmp = find(post> base_mean + 1 * base_sd);
...
%             obj.MEP.duration(int,rep) = numel(tmp)/(obj.ampSettings.SampRate*0.001);
            ...
            if obj.MEP.duration(int,rep) >= 5 %ms
                ...
                iMEP_max  = max(post);
                iMEP_min  = min(post);
                iMEP_ptp  = iMEP_max + abs(iMEP_min);
                obj.MEP.peakToPeak(int,rep)         = (iMEP_ptp-base_ptp)*obj.MEP.duration(int,rep);
            else
...
%                         io64(obj.objLPT.con, obj.objLPT.address2, 0);
        ...
        function [data,isiTic] = stimTMS_act(obj, amp)
            ...
            if obj.tms_settings.mvc_thresh == 0.5
                contr_stim = round((10-5)*rand(1)+5) ; %[1 3] (upper-lower limit)*randNumber+lower limit
            elseif obj.tms_settings.mvc_thresh == 0.7
                contr_stim = round((4-2)*rand(1)+2) ; %[1 3] (upper-lower limit)*randNumber+lower limit
            elseif obj.tms_settings.mvc_thresh == 0.9
                contr_stim = 2; %round((2-1)*rand(1)+1) ; %[1 3] (upper-lower limit)*randNumber+lower limit
            end
            obj.clear_buffer(obj)   %empty buffer
            step_tic = tic;         % start step timer
            while 1 %Stimulate if MVC +- range is reached
                if toc(step_tic) >= 0.2 %take last 200ms
                    ...
                    if  contr_force/obj.mvc.mean > obj.tms_settings.mvc_thresh-obj.tms_settings.mvc_thresh_range ...
                            && contr_force/obj.mvc.mean < obj.tms_settings.mvc_thresh+obj.tms_settings.mvc_thresh_range
                        contr_nb = contr_nb + 1;
                    else %Set count back to 0 as continous contraction
                        contr_nb = 0;
                    end
                    if contr_nb == contr_stim %correct continuous contraction for 2-3s (10-15*200ms)
    ...
    methods(Static)
        ...
        function  obj = lz_TMS_v4(dataSub, projectPath, ampSettings, tms_settings)
            % Initialisation
            set_path_vars;
            ...
            obj.directories.EEG         = [projectPath '\data\' dataSub.initials '\EEG\'];
        ...
        function ins = getInstance(dataSub, projectPath, ampSettings, tms_settings)
            ...
            if( ~strcmpi(class(instance), 'lz_TMS_v4') )
                instance = lz_TMS_v4(dataSub, projectPath, ampSettings, tms_settings);
            end
