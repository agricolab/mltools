function [param,stat,yFit,x_vector]=sigm_fit(x,y,fixed_params,initial_params,plot_flag)
...
[BETA,RESID,J,COVB,MSE] = nlinfit(x,y,f,initial_params(logical(bool_vec)));
stat.param=BETA';
...
% free_param_count=0;
for i=1:size(initial_params(logical(bool_vec)),2);
%     if isnan(fixed_params(i))
%         free_param_count=free_param_count+1;
        param(i)=BETA(i); %free_param_count
%     else
...
if plot_flag==1 
    ...
    plot(x,y,'k.',x_vector,f(param,x_vector),'r-')
    xlim(minmax(x_vector))
    yFit=f(param,x_vector);
else
    x_vector=min(x):diff(minmax(x'))/100:max(x);
    yFit=f(param,x_vector);
end
